...
const defaultChat = product.defaultChatAgent || {
	provider: {
		default: { id: 'unigma', name: 'Unigma' },
		enterprise: { id: 'unigma', name: 'Unigma' },
		google: { id: 'unigma', name: 'Unigma' },
		apple: { id: 'unigma', name: 'Unigma' }
	},
	termsStatementUrl: '',
	privacyStatementUrl: '',
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	private selectedThemeId = 'vesper-violet';
	...
	show(): void {
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to unigma"));
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to unigma");
	...
	private _renderPersonalizeSubtitle(container: HTMLElement): void {
		...
		container.append(
			...
			localize('onboarding.personalize.tip.suffix', " to access all unigma commands."),
		);
	}
	...
	private async _applyKeymap(keymapId: string): Promise<void> {
		...
		if (!keymap?.extensionId) {
			return; // unigma default, nothing to install
		}
