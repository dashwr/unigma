 ...
 *
 * Returns `undefined` when a guard rejects the drop: a target without a client
 * resource, a self-reference, or a malformed payload is never droppable. The
 * client resources are compared by identity only.
 *
 * @param data The dragged chat-reference payload (opaque `chatResource`, client `clientResource`, and `title`).
 * @param ownClientResource The drop-target chat's client resource.
 */
