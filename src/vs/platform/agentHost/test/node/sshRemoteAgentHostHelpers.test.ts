...
suite('SSH Remote Agent Host Helpers', () => {
	...
	function makeEndpoint(overrides: Partial<IAgentHostEndpointMetadata> & Pick<IAgentHostEndpointMetadata, 'type' | 'pid' | 'instanceId'>): IAgentHostEndpointMetadata {
		return {
			...
			// The shared schema-v2 parser always spreads `quality` explicitly
			// (even when absent from the input), so default it here too to keep
			// deepStrictEqual comparisons against parser output exact.
			quality: undefined,
			...overrides,
		};
	}
