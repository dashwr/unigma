...
interface IConnectionEntry {
	...
	/**
	 * Optional teardown for the shared-process relay that this entry's
	 * transport is using (currently SSH/WSL). Tracked separately from
...
export class RemoteAgentHostService extends Disposable implements IRemoteAgentHostService {
	 ...
	 * managed entries are not persisted to settings and therefore don't
	...
	private _normalizeEntry(entry: IRemoteAgentHostEntry): IRemoteAgentHostEntry {
		...
		// `normalizedAddress` is the per-transport address policy.
	...
	getEntryByAddress(address: string): IRemoteAgentHostEntry | undefined {
		const normalized = normalizeRemoteAgentHostAddress(address);
		// Check dynamically registered entries first (e.g. managed connections
	...
	reconnect(address: string): void {
		...
		// Managed entries are reconnected by their respective services
	...
	async addManagedConnection(entry: IRemoteAgentHostEntry, connection: IAgentConnection, transportDisposable?: IDisposable, status = RemoteAgentHostConnectionStatus.connected): Promise<IRemoteAgentHostConnectionInfo> {
		...
		// shared-process relay keyed by connectionId is already owned by
	...
	private _reconcileConnections(): void {
		...
		// added via {@link addManagedConnection} live outside
		...
		// seed their metadata first; without this a live managed connection/WSL
