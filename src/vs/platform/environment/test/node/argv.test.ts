...
suite('parseArgs', () => {
	...
	test('cloud and browser-agent commands are not accepted as subcommands', () => {
		for (const command of ['tunnel', 'serve-web', 'agent']) {
			assert.strictEqual(Object.prototype.hasOwnProperty.call(OPTIONS, command), false);
			assert.deepStrictEqual(parseArgs([command], OPTIONS), { _: [command] });
		}
	});
