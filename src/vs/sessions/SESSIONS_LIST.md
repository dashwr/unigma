...
- Read/unread state is **owned by the sessions provider** and surfaced via `ISession.isRead`. Marking happens through `ISessionsManagementService.markRead` / `markUnread` / `markAllRead`, which route to the provider's `setSessionReadState`. The Copilot Chat provider uses its agent session model (`setRead`); the local chat provider uses its persisted session metadata.
- A provider may persist and publish read state through its own session model. The **editor window** shares that state via the item controller (`IChatSessionsService.canSetChatSessionItemRead` / `setChatSessionItemRead`), mirroring the archive bridge, so marking a session read in either window shows up in the other.
...
- A session becomes **unread** when it produces new output in the background — a turn completes, is cancelled, or errors while the session is not being viewed. Each provider detects this and marks its own session unread: the local chat provider via its tracked session model and the Copilot Chat provider on the `InProgress` → terminal transition. `SessionsService` keeps the **active** agent-window session marked read, while `AgentSessionsModel` writes unsolicited unread state back to an owning provider when the session remains open in an editor-window chat widget. A deliberate **Mark as Unread** is preserved.
...
- Pin/sort state is cleaned up when a provider reports a real session removal; transient provider disconnects may hide cached sessions without reporting them as removed
