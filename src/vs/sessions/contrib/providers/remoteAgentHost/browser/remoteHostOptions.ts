...
/**
 * Whether `preferenceKey` identifies an SSH host that can have a preferred
 * agent run location. WebSocket and WSL addresses are not stable/dedicated-host
 * capable targets for {@link IRemoteAgentHostLocationPreferenceService}.
 *
 * `preferenceKey` must be the host's *stable* preference key (e.g. an SSH
 * host's `ssh:<alias>` from `computeSSHConnectionKey`),
 ...
 * in `sessions.desktop.main.ts`), so this always reports `false` on web regardless of
...
export interface IChangeRemoteAgentHostLocationPreferenceOptions {
	 ...
	 * `ssh:<alias>` key). NOT the live connection
	 ...
	 * SSH entries) but has no corresponding provider right
 ...
 * {@link reconnectRemoteHost} (respecting provider-specific SSH/WSL
...
export async function showRemoteHostOptions(accessor: ServicesAccessor, provider: IAgentHostSessionsProvider, options: IShowRemoteHostOptionsOptions = {}): Promise<'back' | undefined> {
	...
	// desktop (see sessions.desktop.main.ts), so avoid resolving an unregistered
	...
	// separate stable identity (for example WSL).
