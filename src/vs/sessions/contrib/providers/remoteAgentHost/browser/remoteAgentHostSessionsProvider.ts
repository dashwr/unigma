...
export interface IRemoteAgentHostSessionsProviderConfig {
	...
	/** Optional hook to establish a managed connection on demand. */
	readonly connectOnDemand?: () => Promise<void>;
	/** Optional hook to tear down the active managed connection on demand. */
...
export class RemoteAgentHostSessionsProvider extends BaseAgentHostSessionsProvider {
	 ...
	 * Managed providers use their transport hook; other providers fall
	...
	/**
	 * Tear down the active connection for this host. Managed providers use
	 * their transport hook; other providers fall back to the generic remote
	 ...
	 * unreachable (for example, an SSH reconnect failed). The in-memory
	...
	private async _browseForFolder(): Promise<ISessionWorkspace | undefined> {
		// Establish connection on demand if a managed transport hook is provided.
