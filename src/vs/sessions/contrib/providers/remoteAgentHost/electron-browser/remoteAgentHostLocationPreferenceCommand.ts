...
export interface IRemoteAgentHostLocationTarget {
	 ...
	 * and read back by `SSHRemoteAgentHostService` (e.g. `ssh:<alias>` from
	 * {@link computeSSHConnectionKey}). Distinct from {@link address}, which is the
	 * live, connection-specific endpoint.
	 */
	readonly preferenceKey: string;
	/**
	 * The live provider/connection address used to resolve the currently
	 * registered {@link IAgentHostSessionsProvider} (e.g. an SSH host's forwarded
...
/**
 * Enumerate the known SSH remote hosts as preference targets, deduplicated by
 * stable preference key. WebSocket and WSL entries are not preference targets.
 */
...
registerAction2(class extends Action2 {
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const targets = collectRemoteAgentHostLocationTargets(remoteAgentHostService.configuredEntries);
		if (targets.length === 0) {
