...
suite('RemoteAgentHostSessionsProvider', () => {
	...
	test('remoteLocationPreferenceKey defaults to the live address when no stable preference key is given (e.g. WSL)', () => {
		const provider = createProvider(disposables, connection, { address: 'wsl:Ubuntu' });
		assert.strictEqual(provider.remoteLocationPreferenceKey, 'wsl:Ubuntu');
	});
