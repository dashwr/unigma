...
AppPublisher=unigma contributors
AppPublisherURL=https://github.com/dashwr/unigma
AppSupportURL=https://github.com/dashwr/unigma
AppUpdatesURL=https://github.com/dashwr/unigma
...
[InstallDelete]
Type: files; Name: "{app}\bin\unigma-tunnel.exe"
Type: files; Name: "{app}\bin\code-tunnel.exe"
...
Source: "bin\{#ApplicationName}.exe"; DestDir: "{code:GetDestDir}\bin"; DestName: "{code:GetBinDirApplicationFilename}"; Flags: ignoreversion
...
#define LegacyTunnelRunRootKey "HKCU"
#else
#define SoftwareClassesRootKey "HKLM"
#define LegacyTunnelRunRootKey "HKLM"
#endif

Root: {#LegacyTunnelRunRootKey}; Subkey: "Software\Microsoft\Windows\CurrentVersion\Run"; ValueType: none; ValueName: "unigma-tunnel"; Flags: deletevalue
Root: {#LegacyTunnelRunRootKey}; Subkey: "Software\Microsoft\Windows\CurrentVersion\Run"; ValueType: none; ValueName: "code-tunnel"; Flags: deletevalue
...
procedure StopLegacyTunnel(ExecutableName: String);
var
  ResultCode: Integer;
begin
  if FileExists(ExpandConstant('{app}\bin\' + ExecutableName)) then
  begin
    Log('Stopping legacy tunnel executable ' + ExecutableName);
    if not Exec(ExpandConstant('{sys}\taskkill.exe'), '/F /IM "' + ExecutableName + '"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode) then
      Log('Unable to start taskkill for ' + ExecutableName)
    else if (ResultCode <> 0) and (ResultCode <> 128) then
      Log('taskkill for ' + ExecutableName + ' exited with code ' + IntToStr(ResultCode));
  end;
end;
...
begin
  StopLegacyTunnel('unigma-tunnel.exe');
  StopLegacyTunnel('code-tunnel.exe');
  Result := '';
end;
...
begin
  if IsBackgroundUpdate() and IsVersionedUpdate() then
    Result := ExpandConstant('new_{#ApplicationName}.exe')
  else
    Result := ExpandConstant('{#ApplicationName}.exe');
end;
