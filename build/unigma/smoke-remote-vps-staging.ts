...
async function probeNativeModules(runner: (arguments_: readonly string[]) => RemoteSshProcess, destination: string, controlPath: string, commit: string): Promise<void> {
	const probe = await runCommand(runner, buildNativeProbeArguments(destination, controlPath), buildNativeProbeScript(commit, buildRemoteServerPathShellFragments().versionedDirectory));
