...
// `@github/copilot` is a thin launcher and the real binaries live in
// `@github/copilot-<platform>-<arch>`. `npm install` / `npm ci`
...
//   - Product build jobs run it after `npm ci` so a poisoned node_modules cache
//     never hides a missing native package for a shipped integration.
 ...
 *
 * A base package (e.g. `@github/copilot`) ships its native binary in a
 * per-platform optional dependency named `<base>-<target>` (e.g.
 * `@github/copilot-linux-x64`, `@github/copilot-linuxmusl-x64`).
 ...
 * package.
 *
 * `target` is the package suffix — e.g. `darwin-arm64`, `linux-x64`, or
 * `linuxmusl-x64`.
 *
...
function main(): void {
	...
	const target = platform === 'linux' && process.env['npm_config_libc'] === 'musl'
		? `linuxmusl-${arch}`
		: `${platform}-${arch}`;
	const errors: string[] = [];
