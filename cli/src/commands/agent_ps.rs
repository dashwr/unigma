...
///
/// With an explicit `--address`, this queries exactly one host
...
pub async fn agent_ps(ctx: CommandContext, args: AgentPsArgs) -> Result<i32, AnyError> {
	if args.discovery.address.is_some() {
...
/// Single-host path used for an explicit `--address`.
async fn agent_ps_single(
	ctx: &CommandContext,
	args: &AgentPsArgs,
	address: Option<&str>,
) -> Result<i32, AnyError> {
	let client = agent::connect_explicit(ctx, address).await?;
