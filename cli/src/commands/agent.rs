...
/// Connects to an agent host at an explicit `--address` target, initializes
/// the AHP session, and returns the ready-to-use client.
///
...
/// passing no `address` is a caller bug, not a "no target given" case to
...
pub async fn connect_explicit(
	...
	let client = match address {
		Some(addr) => connect_ws(addr).await?,
		None => unreachable!(
			"connect_explicit requires --address; callers must route the no-target \
			case through agent_discovery instead"
		),
	};
...
/// abstraction. This is how the CLI reaches `editor`-owned
