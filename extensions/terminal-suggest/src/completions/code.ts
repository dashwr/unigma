...
const disabledProductSubcommands = new Set(['tunnel', 'serve-web', 'agent']);
const codeSubcommands = codeTunnelSubcommands
	.filter(command => typeof command.name !== 'string' || !disabledProductSubcommands.has(command.name))
	.map(command => command.name === 'help'
		? { ...command, subcommands: command.subcommands?.filter(child => typeof child.name !== 'string' || !disabledProductSubcommands.has(child.name)) }
		: command);
