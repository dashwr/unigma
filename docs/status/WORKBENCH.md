...
| decisão/poda | `Q-3` / `CLI-003` | poda local concluída em 2026-09-02: extensão `tunnel-forwarding`, specs dos binários `code-tunnel`, entradas i18n de diretórios inexistentes e referências de build/fixture removidas; Code Server/RPC e port forwarding genérico preservados | colher validação oficial no runner e revisar o delta de distribuição |
...
| 2026-09-02 | `CLI-003` / `Q-3` | auditoria → implementação/review | por autorização explícita, a extensão morta, suas entradas de build/lint, as specs `code-tunnel`/`code-tunnel-insiders`, as entradas i18n inexistentes e os caminhos do fixture URI foram removidos; a guarda nominal do auditor foi mantida para rejeitar artefatos residuais, sem tocar em `cli/` | validar checks focados e runner |
