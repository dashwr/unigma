...
| `CLI-002` | etapa B: OpenSSH → `unigma-server` remoto → extension host → `unigma-agent-runtime` → `opencode serve` (T-050…T-053) | implementação/review | `review` | integrar staging ao fluxo de autoridade e colher a matriz SSH no runner; conexão já passa no runner, staging aguarda | `CLI-001`, `D-028`, `D-032`, host autorizado | [`../planos/2026-08-29-cli-ssh-remoto.md`](../planos/2026-08-29-cli-ssh-remoto.md), [`../SSH-CONTRACT.md`](../SSH-CONTRACT.md) |
