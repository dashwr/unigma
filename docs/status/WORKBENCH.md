...
| implementação | `T-052` / `AC-007` | falta a ativação remota (extrair o payload e ativar atomicamente) e a fiação do resolver, que ainda devolve sempre `NotAvailable`. Transporte, par, manifesto, plano de staging e smoke de conexão já existem e passam no runner. Não é mais bloqueio de ambiente | próxima frente |
