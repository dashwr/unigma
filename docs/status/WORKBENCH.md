...
sem executar teste algum — e foi corrigido. O run **`34081018988` fecha a
cobertura**: suíte do runtime 178 com 1 pendente, `test/common` 47 e
`test/browser` 13 em Electron sob Xvfb, com o smoke desktop de volta a 40
passando e 0 falhando. As três suítes do `unigmaAgent` executam em CI pela
primeira vez.
...
| --- | --- | --- | --- |
| decisão | `T-054` / rota da bancada efêmera | o recorte agentivo tem uma rota que **não depende de autorização externa**, e o handoff dizia o contrário. `smoke-remote-staging.ts` sobe um `sshd` efêmero em `127.0.0.1` dentro do WSL do runner, e `unigma-remote-staging-smoke.yml` está na branch default. Contra essa bancada não há escrita em host de terceiro (o filesystem é o do próprio WSL, que já tem o checkout) nem VPS. A observação dispensa contrato novo: o driver afirma o efeito — `opencode serve` rodando naquele host, loopback daquele host respondendo — em vez dos eventos, que o `OutputChannel` não expõe. Falta escrever ~400–500 linhas: driver mínimo, smoke e um passo no workflow. **Decidir se vale**, sabendo que verde ali não fecha `AC-007`, cuja matriz pede host real | pendente |
| risco/CI | `unigma-remote-window-smoke.yml` na branch default | a versão em `main` — a única que a UI do GitHub oferece — tem só o input `alias` e **um único job sem guarda**, que provisiona a VPS e a limpa com `if: always()`. As guardas `reconnect_only`/`native_modules_only` existem apenas em `remote-runtime`. Um dispatch **por CLI com `--ref`** usa a definição do ref e portanto respeita as guardas — medido no run `34082375469` —, mas quem usar a UI na branch default só consegue pedir o caminho que provisiona e limpa. Levar a versão com guardas para `main`, e **não só ela**: o job de roteamento usa `./.github/workflows/unigma-remote-native-modules-smoke.yml` e `./.github/workflows/unigma-remote-reconnect-smoke.yml`, que não existem lá, e um `uses:` para arquivo ausente invalida o workflow inteiro. Os scripts que eles chamam também faltam em `main`: `build/unigma/smoke-remote-native-modules.ts`, `build/unigma/remote-native-probe.ts` e `build/unigma/remote-window-ssh-proxy.ts`. Um dispatch com `--ref` de branch de trabalho não precisa deles em `main`, porque o checkout vem do ref; um dispatch pela UI na branch default precisa | pendente |
...
| 2026-09-07 | `T-054` / staging do par | o OpenCode podia ser ativado sem bit de execução | `9016876d`. O script de staging exigia `[ -x ]` do `unigma-server` e nada do `opencode`; o manifesto fixa tamanho e hash, e nenhum carrega o modo, enquanto a extração usa `--no-same-permissions` de propósito. A ativação passava e a falha aparecia só depois, no host, como o runtime recusando o próprio bundle. Passa a falhar cedo, com categoria `opencode-not-executable`; a guarda recusa e não conserta, e há teste que proíbe `chmod` no script. No mesmo commit, a resolução dos dois layouts deixou de estar provada só contra filesystem falso: três testes passam por `ChildProcessManager` com diretórios reais, incluindo o binário sem bit de execução. `5f865511` afirma a mesma propriedade na montagem e na verificação do payload, de modo que ela passa a valer em montagem, verificação, staging e runtime |
| 2026-09-07 | CI / semântica de `workflow_dispatch` | medido, não inferido | run `34082375469`. A dúvida era se um dispatch com `--ref` usa a definição do ref ou a da branch default — o que decide se um modo somente-leitura que só existe numa branch de trabalho pode ser selecionado. Um input presente apenas em `remote-runtime` foi acrescentado ao `unigma-windows-ssh-capabilities.yml`, que não conecta a host algum: o dispatch foi **aceito**, logo a validação usa o ref, e o job imprimiu `definition=feature-branch`, logo a execução também. A branch default só precisa ter o arquivo para o workflow ser despachável. O andaime foi revertido |
| 2026-09-07 | `T-054` / pacote do servidor | o runtime viajava como manifesto sozinho | runs `34086194290` e `34087643005`. `unigma-agent-runtime` estava no pacote do servidor e era `workspace`, mas sem o ponto de entrada: a listagem do diretório empacotado era `[package.json]`. O extension host remoto não tinha o que carregar, o que é anterior a todo o resto do recorte agentivo. Causa: a extensão não tem config esbuild, então o empacotamento toma `fromLocalNormal`, que é `vsce.listFiles` sobre o disco; o job desktop compila antes e o do servidor não compilava, com a árvore do WSL recriada a cada run sem `extensions/**/out/`. Corrigido em `a39f952f`. Ninguém via porque `auditExtensionEntryPoints` só era chamado no ramo desktop |
| 2026-09-07 | `audit-distribution --server` | o pacote do servidor passa a afirmar que o runtime remoto está nele | `5b1b5d04`. Toda a história do agente remoto depende de `unigma-agent-runtime` estar no pacote do servidor e ser `workspace`, e nada afirmava nem uma coisa nem outra. A metade negativa já custara caro — `gulpfile.reh.ts` comparava `extensionKind` com string quando é array, e embarcava toda extensão com `main` —, mas a positiva ficou descoberta: sem o runtime o servidor compila, responde `/version` e passa na auditoria. Três checks novos, exercitados contra pacotes sintéticos nos três sentidos. Elo da cadeia de `AC-007`, não prova dele |
| 2026-09-07 | `audit-remote-safety` | a limpeza da árvore temporária ganha prova de runner, e a regra vale para a classe | `f95ca3e2`. O conserto de `acd6d274` vivia num workflow que não é despachável, então não tinha run. A propriedade passou para `audit-remote-safety.ts`, que aplica regras de texto sobre shell montado como string — a classe de código que nenhum type checker vê — e cuja suíte roda no runner: `mktemp -d` sob `$HOME`, `trap cleanup EXIT`, sentinela vazia, guarda de prefixo, e a remoção guardada sendo a única do arquivo. Junto, uma regra para todos os workflows: remoção recursiva nunca nomeia `$HOME` nem a raiz, e nunca deixa a expansão sem aspas, porque uma variável vazia transforma `rm -rf $tree/out` em `rm -rf /out`. 104 testes em `build/unigma` |
| 2026-09-07 | cobertura do `unigmaAgent` | as três suítes executam em CI | run `34081018988`, head `ca5dd98b`: runtime 178 com 1 pendente, `test/common` 47, `test/browser` 13 em Electron sob Xvfb, contrato RPC 7, e smoke desktop 40 passando com 0 falhando. Cada número tem nome de teste no log, não só passo verde. O run anterior, `34079783571`, tinha caído no smoke porque o passo novo deixava um `out` só transpilado e `ensureCompiled` em `build/lib/preLaunch.ts` só compila quando `out` está ausente; o passo passou a devolver a árvore como a encontrou |
...
| 2026-09-07 | cobertura de teste | a suíte de browser do `unigmaAgent` passa a rodar em CI | ela nunca foi executada por workflow algum e estava vermelha em dois pontos: a asserção sobre `UNIGMA_AGENT_VIEW_STATES` esperava quatro estados, sem `running`, e um literal `version: 2` era disparado esperando recusa do envelope — o que deixou de ser verdade quando `ace14a12` tornou v2 a versão suportada; a versão inválida passa a ser derivada da constante. O job Linux ganhou um passo em Electron sob Xvfb contra a árvore `out` transpilada. 13 passando, verificado em Electron na máquina de desenvolvimento antes de ir ao runner |
...
| 2026-09-04 | `THEME-001` / `AC-011` | alto contraste próprio implementado → `review` | `theme-unigma` agora contribui `unigma High Contrast`, com editor/workbench/terminal, tokens de sintaxe e cores sólidas de foco/borda; o guard local passou com editor/terminal `21,00:1`, foco `11,94:1` e seleção com diferença de luminância `0,116`. Runner e revisão visual continuam pendentes |
