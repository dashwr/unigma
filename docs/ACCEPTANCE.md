...
| AC-007 | RQ-006 | A especificação de implementação define o fluxo SSH suportado e um teste estabelece ou recusa a conexão conforme a política definida. | parcial/bloqueado: conexão remota tem smoke passando no runner; staging/ativação e confirmação fail-closed estão implementados mas só têm execução local, que não é aceite; resolver integrado e matriz oficial continuam ausentes |
