...
| AC-007 | RQ-006 | A especificação de implementação define o fluxo SSH suportado e um teste estabelece ou recusa a conexão conforme a política definida. | parcial/bloqueado: conexão remota e staging/ativação com payload real têm smoke passando no runner, com confirmação fail-closed; falta a fiação do resolver, que ainda recusa toda autoridade, e a matriz oficial |
