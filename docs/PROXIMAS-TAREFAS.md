...
| [EM REVISÃO] T-011 / E01 | Provider e modelo autorizados por D-043; secret criado; smoke `smoke-opencode-provider.ts` escrito e no workflow Linux | `connected` não prova credencial; só prompt respondido fecha. Falta prova de runner |
...
| [EM REVISÃO] TD1 spdlog | **Fechado no código por `3d37d0d2`**, verificado em 2026-09-06: existe fallback de console, o buffer é drenado nos dois caminhos e `log()` respeita o fallback. Falta a prova de runner com o addon realmente indisponível | Não reabrir por leitura da linha antiga; prova é falha reproduzida do addon, não teste puro |
